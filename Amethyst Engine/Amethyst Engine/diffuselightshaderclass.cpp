...
DiffuseLightShaderClass::DiffuseLightShaderClass()
{
	h_effect = 0;
	h_technique = 0;
	h_layout = 0;
...
DiffuseLightShaderClass::DiffuseLightShaderClass(const DiffuseLightShaderClass& other)
{
}
...
bool DiffuseLightShaderClass::Initialize(ID3D10Device *device, HWND hwnd)
{
	bool result;

	result = InitializeShader(device, hwnd, L"../Amethyst Engine/diffuselight.fx");
	if(!result)
	{
		return false;
	}
...
void DiffuseLightShaderClass::Shutdown()
{
	ShutdownShader();
...
void DiffuseLightShaderClass::Render(ID3D10Device *device, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D10ShaderResourceView *texture, D3DXVECTOR3 lightDirection, D3DXVECTOR4 diffuseColor)
{
...
bool DiffuseLightShaderClass::InitializeShader(ID3D10Device *device, HWND hwnd, WCHAR *filename)
{
	HRESULT result;
	...
	h_technique =  h_effect->GetTechniqueByName("DiffuseLightTechnique");
	if(!h_technique)
	{
		return false;
	}
...
void DiffuseLightShaderClass::ShutdownShader()
{
	h_lightDirectionPtr = 0;
	h_diffuseColorPtr = 0;
	h_texturePtr = 0;
...
void DiffuseLightShaderClass::OutputShaderErrorMessage(ID3D10Blob *errorMessage, HWND hwnd, WCHAR *shaderFilename)
{
	char *compileErrors;
...
void DiffuseLightShaderClass::SetShaderParameters(D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D10ShaderResourceView *texture, D3DXVECTOR3 lightDirection, D3DXVECTOR4 diffuseColor)
{
...
void DiffuseLightShaderClass::RenderShader(ID3D10Device *device, int indexCount)
{
